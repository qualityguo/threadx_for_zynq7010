...
*********************************************************************************************************
*/
UCHAR data_buffer2[4096];

NX_PACKET_POOL    pool_2;
NX_IP             ip_2;
...
ULONG  packet_pool_area2[NX_PACKET_POOL_SIZE/4 + 4];

/* ARP缓存 */
ULONG    arp_space_area2[512 / sizeof(ULONG)];
ULONG    error_counter2;
...
void NetXTest2(void)
{
    ...
    /* 创建内存池 */
    status =  nx_packet_pool_create(&pool_2,                                       /* 内存池控制块 */
                                     "NetX Main Packet Pool",                      /* 内存池名 */
                                     1536,                                         /* 内存池每个数据包大小，单位字节
                                                                                      此值必须至少为 40 个字节，并且还必须可以被 4 整除 */
									 (ULONG*)(((int)packet_pool_area2 + 15) & ~15) ,/* 内存池地址，此地址必须ULONG对齐 */
                                     NX_PACKET_POOL_SIZE);                         /* 内存池大小 */

    /* 检测创建是否失败 */
    if (status) error_counter2++;

    /* 例化IP */
    status = nx_ip_create(&ip_2,                                                   /* IP实例控制块 */
                            "NetX IP Instance 2",                                  /* IP实例名 */
    ...
    /* 使能ARP，并提供ARP缓存 */
    status =  nx_arp_enable(&ip_2,                     /* IP实例控制块 */
							(void *)arp_space_area2,    /* ARP缓存地址 */
							sizeof(arp_space_area2));   /* 每个 ARP 条目均为 52 个字节，因此，ARP 条目总数是52字节整数倍 */
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////
    /* 创建TCP Socket */
    ret = nx_tcp_socket_create(&ip_2,                 /* IP实例控制块 */
                               &TCPSocket2,            /* TCP控制块 */
    ...
    */
    ret = nx_tcp_server_socket_listen(&ip_2,                  /* IP实例控制块 */
									  DEFAULT_PORT,           /* 默认端口 */
	                                  &TCPSocket2,             /* TCP Socket控制块 */
	...
	/* 启动TCP通信前，接收新连接 */
	ret = nx_tcp_server_socket_accept(&TCPSocket2,         /* TCP Socket控制块 */
	                                   TX_WAIT_FOREVER);  /* 监听回调函数 */

	if (ret)
	{
		app_printf("nx_tcp_server_socket_accept error!\n");
	}


	while(1)
	{
        TX_MEMSET(data_buffer2, '\0', sizeof(data_buffer2));

        /* 获取socket状态 */
        nx_tcp_socket_info_get(&TCPSocket2,     /* TCP Socket控制块 */
		                       NULL,           /* 发送的TCP数据包总数目 */
		                       NULL,           /* 发送的TCP总字节数 */
		                       NULL,           /* 接收TCP数据包总数目 */
		                       NULL,           /* 接收的TCP总字节数 */
        ...
        if((socket_state == NX_TCP_ESTABLISHED)&&(ret == NX_SUCCESS))
        {
            ...
            /* 接收TCP客户端发的TCP数据包 */
            ret = nx_tcp_socket_receive(&TCPSocket2,        /* TCP Socket控制块 */
			                            &data_packet,      /* 接收到的数据包 */
            ...
            if (ret == NX_SUCCESS)
            {
				...
				/* 获取客户端的IP地址和端口 */
				nx_tcp_socket_peer_info_get(&TCPSocket2,       /* TCP Socket控制块 */
                ...
                /* 打印接收到数据 */
                PRINT_DATA(peer_ip_address, (unsigned int)peer_port, data_buffer2);

                /* 立即将接收到的数据发送回去 */
                ret =  nx_tcp_socket_send(&TCPSocket2,       /* TCP Socket控制块 */
				                          data_packet,      /* 数据包 */
				                          NX_WAIT_FOREVER); /* 永久等待 */
            }
            else
            {
                /* 断开连接 */
                nx_tcp_socket_disconnect(&TCPSocket2,
				                         NX_WAIT_FOREVER);

				/* 解除Socket和服务器端口的绑定 */
                nx_tcp_server_socket_unaccept(&TCPSocket2);

				/* 重新监听 */
                nx_tcp_server_socket_relisten(&ip_2,
				                              DEFAULT_PORT,
				                              &TCPSocket2);
            }
        }

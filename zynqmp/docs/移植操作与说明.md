...
- `/threadx/ports/cortex_a53/gnu/src/*`到`ThreadX/Port`目录下（拷贝文件）
- `/threadx/ports/cortex_a53/gnu/inc/tx_port.h`到根目录下的`ThreadX/tx_port.h`
- `/threadx/ports/cortex_a53/gnu/example_build/*.h&*.S`到`ThreadX/Port`目录下（拷贝文件）
- `/threadx/utlity/execution_profile_kit/*.s&*.c`到`ThreadX/utlity/execution_profile_kit/`目录下（拷贝文件）
- 删除无关代码
  - `startup.S`：boot启动代码直接采用XilinxBSP实现
...
- 加入一段汇编函数`tx_thread_time_get.S`

  - ```
    // 用作获取全局定时器的计数值-性能测试
    	.text
        .align 3
        .global _tx_thread_time_get
        .type  _tx_thread_time_get, @function
    _tx_thread_time_get:
        MRS     x0, CNTPCT_EL0
        UBFX    x0, x0, #0, #32
        RET
...
- 修改`tx_user.h`
  ...
  - ```
    #define TX_ENABLE_STACK_CHECKING			// 使能栈填充
    #define TX_ENABLE_EVENT_TRACE
    #define TX_TRACE_TIME_SOURCE                    _tx_thread_time_get();
    #define TX_TRACE_TIME_MASK                      0xFFFFFFFFUL				// 使能TraceX
    #define TX_EXECUTION_PROFILE_ENABLE											// 是性能检测
    ```

- 解决ThreadX的宏定义和Xilinx-BSP的宏定义的冲突
  ...
  - 并在包含`xil_type.h`之前包含`tx_api.h`即可，否则还会报错，无法解决

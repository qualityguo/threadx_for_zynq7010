...
volatile double 				OSCPUUsage[TX_THREAD_SMP_MAX_CORES];       	   	/* CPU百分比 */
...
static  void  AppTaskStart (ULONG thread_input)
{
	...
	EXECUTION_TIME ThreadTime_S2[TX_THREAD_SMP_MAX_CORES];
	EXECUTION_TIME IdleTime_S2[TX_THREAD_SMP_MAX_CORES];
	EXECUTION_TIME IsrTime_S2[TX_THREAD_SMP_MAX_CORES];
	EXECUTION_TIME deltaThreadTime[TX_THREAD_SMP_MAX_CORES];
	EXECUTION_TIME deltaIsrTime[TX_THREAD_SMP_MAX_CORES];
	EXECUTION_TIME deltaIdleTime[TX_THREAD_SMP_MAX_CORES];
	uint32_t uiCount = 0;
	(void)thread_input;
    ...
    while (1)
	{
    	...
    	if(uiCount == 200)
    	{
    		uiCount = 0;
    		/* 记录S2时间 */
    		_tx_execution_thread_total_time_get(&ThreadTime_S2[0]);
			_tx_execution_isr_time_get(&IsrTime_S2[0]);
			_tx_execution_idle_time_get(&IdleTime_S2[0]);

			/* 计算差值 */
			for(UINT i = 0; i < TX_THREAD_SMP_MAX_CORES; i++)
			{
				deltaThreadTime[i] = ThreadTime_S2[i] - ThreadTime_S1[i];
				deltaIsrTime[i] = IsrTime_S2[i] - IsrTime_S1[i];
				deltaIdleTime[i] = IdleTime_S2[i] - IdleTime_S1[i];
				OSCPUUsage[i] = (double)deltaIdleTime[i]/(deltaThreadTime[i] + deltaIsrTime[i] + deltaIdleTime[i]);
				OSCPUUsage[i] = 100 - OSCPUUsage[i]*100;
			}
...
{
	/* 设置任务核依赖-CPU0 */
	tx_thread_smp_core_exclude(&AppTaskStartTCB, TASK_EXC_CORE1);
	tx_thread_smp_core_exclude(&AppTaskLEDTCB, TASK_EXC_CORE1);
	tx_thread_smp_core_exclude(&AppTaskMsgRecvTCB, TASK_EXC_CORE1);
	tx_thread_smp_core_exclude(&AppTaskSemRecvTCB, TASK_EXC_CORE1);
	/* 设置任务核依赖-CPU1 */
	tx_thread_smp_core_exclude(&AppTaskKEYTCB, TASK_EXC_CORE0);
	tx_thread_smp_core_exclude(&AppTaskUartRecvTCB, TASK_EXC_CORE0);
}

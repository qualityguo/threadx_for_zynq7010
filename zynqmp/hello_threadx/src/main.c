...
int main()
{
	xil_printf("Hello Threadx for ZYNQMP!\r\n");
...
*********************************************************************************************************
*/
void uart0_cb(struct device *dev, uint32_t event)
{
...
static  void  AppTaskUartRecv		(ULONG thread_input)
{
	(void)thread_input;
	UINT status;
	struct device *puart0 = device_find("uart0");
	device_ioctl(puart0, DEV_IOCTL_SET_NOTIFY, uart0_cb);
	int n = 0;
	...
	while(1)
	{
		...
		if(status == TX_SUCCESS)
		{
			n = device_read(puart0, tmp_buf, sizeof(tmp_buf));
			App_Printf("串口接收数据长度:%d\r\n", n);
			device_write(puart0, tmp_buf, n);
...
static void DispTaskInfo(void)
{
	...
	while (p_tcb != (TX_THREAD *)0)
	{
		...
		App_Printf("   %2d        %5d      %5d       %5d      %s\r\n",
                    ...
                    (uintptr_t)p_tcb->tx_thread_stack_end - (uintptr_t)p_tcb->tx_thread_stack_ptr,
                    (uintptr_t)p_tcb->tx_thread_stack_end - (uintptr_t)p_tcb->tx_thread_stack_highest_ptr,

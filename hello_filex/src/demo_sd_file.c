...
void DemoFileX(char cmd)
{
	...
	{
		case '1':
			xil_printf("【1 - ViewRootDir】\r\n");
			ViewRootDir();		/* 显示SD卡根目录下的文件名 */
			break;

		case '2':
			xil_printf("【2 - CreateNewFile】\r\n");
			CreateNewFile();	/* 创建一个新文件,写入一个字符串 */
			break;

		case '3':
			xil_printf("【3 - ReadFileData】\r\n");
			ReadFileData();		/* 读取根目录下armfly.txt的内容 */
			break;

		case '4':
			xil_printf("【4 - CreateDir】\r\n");
			CreateDir();		/* 创建目录 */
			break;

		case '5':
			xil_printf("【5 - DeleteDirFile】\r\n");
			DeleteDirFile();	/* 删除目录和文件 */
			break;

		case '6':
			xil_printf("【6 - TestSpeed】\r\n");
			WriteFileTest();	/* 速度测试 */
			break;
...
{
	xil_printf("\r\n------------------------------------------------\r\n");
	xil_printf("请选择操作命令，打开SD卡模拟U盘操作期间不支持再调用命令1-6:\r\n");
	xil_printf("1 - 显示根目录下的文件列表\r\n");
	xil_printf("2 - 创建一个新文件armfly.txt\r\n");
	xil_printf("3 - 读armfly.txt文件的内容\r\n");
	xil_printf("4 - 创建目录\r\n");
	xil_printf("5 - 删除文件和目录\r\n");
	xil_printf("6 - 读写文件速度测试\r\n");
}
...
static void ViewRootDir(void)
{
    ...
    /* 挂载SD卡 */
    status =  fx_media_open(&sdio_disk, "ZYNQ_SDIO_DISK", fx_zynq_sd_driver, 0, media_memory, sizeof(media_memory));

    if (status != FX_SUCCESS)
    {
        xil_printf("挂载文件系统失败 -- %d\r\n", status);
        return;
    }


    xil_printf("属性        |  文件大小 | 短文件名 | 长文件名\r\n");
	for (cnt = 0; ;cnt++)
	{
        /* 读取目录项，索引会自动下移 */
...
static void CreateNewFile(void)
{
    ...
    if (status == FX_SUCCESS)
    {
        xil_printf("armfly.txt 文件写入成功\r\n");
    }
    else
    {
        xil_printf("armfly.txt 文件写入失败 %d\r\n", status);
    }
...
static void CreateDir(void)
{
    ...
    if (status == FX_SUCCESS)
    {
        xil_printf("文件夹Dir1创建成功\r\n");
    }
	else if (status == FX_ALREADY_CREATED)
	{
		xil_printf("Dir1 目录已经存在(%d)\r\n", status);
	}
	else
	{
		xil_printf("fx_directory_create Dir1 失败 (%d)\r\n", status);
		return;
	}
    ...
    if (status == FX_SUCCESS)
    {
        xil_printf("文件夹Dir2创建成功\r\n");
    }
	else if (status == FX_ALREADY_CREATED)
	{
		xil_printf("Dir2 目录已经存在(%d)\r\n", status);
	}
	else
	{
		xil_printf("fx_directory_create Dir2 失败 (%d)\r\n", status);
		return;
	}
    ...
    if (status == FX_SUCCESS)
    {
        xil_printf("文件夹Dir1/Dir1_1创建成功\r\n");
    }
	else if (status == FX_ALREADY_CREATED)
	{
		xil_printf("Dir1/Dir1_1 目录已经存在(%d)\r\n", status);
	}
	else
	{
		xil_printf("fx_directory_create Dir1/Dir1_1 失败 (%d)\r\n", status);
		return;
	}
...
static void DeleteDirFile(void)
{
    ...
    if (status == FX_SUCCESS)
	{
		xil_printf("删除目录Dir1成功\r\n");
	}
	else if (status == FX_NOT_FOUND)
	{
		xil_printf("没有发现文件或目录 :%s\r\n", "/Dir1");
	}
	else
	{
		xil_printf("删除Dir1失败(错误代码 = %d) 文件只读或目录非空\r\n", status);
	}
	...
	{
		xil_printf("删除Dir1/Dir1_1失败(错误代码 = %d) 文件只读或目录非空\r\n", status);
	}
	...
	{
		xil_printf("删除Dir2失败(错误代码 = %d) 文件只读或目录非空\r\n", status);
	}
	...
	{
		xil_printf("删除armfly.txt失败(错误代码 = %d) 文件只读或目录非空\r\n", status);
	}
	...
	for (i = 0; i < 20; i++)
	{
        ...
        {
            xil_printf("删除%s文件失败(错误代码 = %d) 文件只读或目录非空\r\n", path, status);
        }
	}
...
static void WriteFileTest(void)
{
    ...
    if (status != FX_SUCCESS)
    {
        xil_printf("fx_media_flush失败\r\n");
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////
    /* 开始读文件测试 */
	xil_printf("开始读文件 %dKB ...\r\n", TEST_FILE_LEN / 1024);
    ...
    for (i = 0; i < TEST_FILE_LEN / BUF_SIZE; i++)
	{
        ...
        if (status == FX_SUCCESS)
        {
			...
			for (k = 0; k < sizeof(g_TestBuf); k++)
			{
				...
				{
				  	err = 1;
					xil_printf("Speed1.txt 文件读成功，但是数据出错\r\n");
					break;
				}
			}
			if (err == 1)
